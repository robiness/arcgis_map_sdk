...
class WebStreamManager {
  ...
  final StreamGroup<LatLng> _centerPositionStreamGroup = StreamGroup.broadcast();
  final StreamGroup<BoundingBox> _boundsStreamGroup = StreamGroup.broadcast();
  final StreamGroup<String> _attributionTextStreamGroup = StreamGroup.broadcast();
  final StreamGroup<Attributes?> _onClickStreamGroup = StreamGroup.broadcast();
  final StreamGroup<List<String>> _visibleGraphicsStreamGroup = StreamGroup.broadcast();
  ...
  final StreamController<bool> _graphicHoveredController = StreamController.broadcast();
  ...
  BoundingBox _extentToBoundingBox(JsExtent extent) {
    return BoundingBox(
      ...
      width: extent.width,
      topRight: LatLng(
        extent.center.latitude + (extent.height / 2), 
  ...
  Future<void> _handleClickEvent(JSObject event, StreamController<Attributes?> controller, JsView view) async {
    try {
  ...
  Future<void> _handleHoverEvent(JSObject event, StreamController<bool> controller, JsView view) async {
    try {
      JSPromise<JsHitTestResult> hitTestPromise;
